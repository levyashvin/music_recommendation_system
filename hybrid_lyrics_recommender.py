...
def build(csv_path, out_dir, id_col, title_col, artist_col, lyrics_col,
    ...
    all_chunks, doc_ptrs = [], []
    for txt in df[lyrics_col].tolist():
        ch = chunk_words(txt, max_words=max_words) or [txt]
        s = len(all_chunks)
        all_chunks.extend(ch)
        e = len(all_chunks)
        doc_ptrs.append((s, e))

    chunk_emb = model.encode(all_chunks, batch_size=batch_size, convert_to_numpy=True, normalize_embeddings=True)
    dim = chunk_emb.shape[1]
    emb = np.zeros((len(df), dim), dtype=np.float32)
    for i, (s, e) in enumerate(doc_ptrs):
        emb[i] = chunk_emb[s:e].mean(axis=0)
    # Normalize pooled embeddings so dot = cosine for all backends
    ...
    print(f"Built: TF-IDF={X.shape}, Embeddings={emb.shape}, out={out_dir}")
...
def load_runtime(out_dir, model_name=None, set_ef=256, index_backend="annoy", device="auto"):
    ...
    X = load_npz(os.path.join(out_dir, "tfidf_matrix.npz"))
...
def hybrid_search_by_text(out_dir, query, k=10, candidates=1500, tfidf_top=0, alpha=0.6,
    ...
    # TF-IDF query
    q_tfidf = vec.transform([clean_lyrics(query)])
